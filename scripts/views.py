...
class AdvancedMultiExpertAISearchView(APIView):
    ...
    def post(self, request):
        try:
            self.logger.info("=== Multi-Expert AI Search (Unified Service) ===")
            ai_service = get_ai_service()
            if not ai_service:
                return Response({
                    "status": "error",
                    "message": "AI service not available - dependencies not installed",
            ...
            try:
                # Use unified AI service (advanced features included)
                import asyncio
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                results = loop.run_until_complete(
                    ai_service.analyze_image_advanced(
                        image_bytes,
                ...
                return Response({
                    "status": "success",
                    "message": "Unified advanced AI analysis completed",
                    "results": results,
                    "ai_service_used": "unified_advanced",
                ...
                self.logger.error(f"Unified AI analysis failed: {str(e)}")
                return Response({
                    "status": "error",
            ...
            self.logger.error(f"Unified AI search failed: {str(e)}")
            return Response({
                "status": "error",
