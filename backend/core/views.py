...
class SetEbayRefreshTokenView(APIView):
    ...
    def post(self, request):
        ...
        if not new_token:
            return Response({'error': 'Missing ***REMOVED***in request body.'}, status=400)
        try:
    ...
    def get(self, request):
        ...
        try:
            ...
            ***REMOVED*** = getattr(settings, 'EBAY_PRODUCTION_CLIENT_SECRET', None)
            ...
            if not all([client_id, ***REMOVED***]):
                return Response({'error': 'Missing eBay client credentials.'}, status=500)

            token_url = 'https://api.ebay.com/identity/v1/oauth2/token'
            basic_auth = base64.b64encode(f"{client_id}:{***REMOVED***}".encode()).decode()
            headers = {
            ...
            if resp.status_code == 200:
                ...
                ***REMOVED***= token_data.get('refresh_token')
                if refresh_token:
                    from .ebay_auth import token_manager
